		...
		if (client.holder && client.status_tabs)
			if (statpanel("Status"))
				...
				if (processes.time_track)
					stat("CPU (Average) (Movement Scheduler):","[world.cpu]% ([ceil(processes.time_track.stored_averages["cpu"])]%) ([ceil(movementMachine.average_cpu)]%)")
					stat("Tick Usage (Average) (Movement Scheduler):","[ceil(world.tick_usage)]% ([ceil(processes.time_track.stored_averages["tick_usage"])]%) ([ceil(movementMachine.average_tick_usage)]%)")
