...
/proc/move_multi_shuttles()

	var/obj/machinery/computer/shuttle_control/multi/computer = locate(/obj/machinery/computer/shuttle_control/multi)

	if (!computer)
		return

	var/datum/shuttle/multi_shuttle/MS = shuttle_controller.shuttles[computer.shuttle_tag]
	if(!istype(MS)) return
	...
	if(!computer.check_docking(MS))
		return

	var/choice = input("Select a destination.") as null|anything in MS.destinations
	if(!choice) return

	usr << "\blue [computer.shuttle_tag] main computer recieved message."
