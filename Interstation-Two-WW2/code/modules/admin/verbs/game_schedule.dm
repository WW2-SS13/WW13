...
/client/proc/forceClose_game_schedule()
	...
	var/i = input("Forcibly off the game schedule? Current forceClosed setting is [global_game_schedule.forceClosed]. (If 1, selecting 'No' will disable it.)") in list("Yes", "No")
	if (i == "Yes")
...
/client/proc/forceOpen_game_schedule()
	...
	var/i = input("Forcibly open the game schedule? Current forceOpened setting is [global_game_schedule.forceOpened]. (If 1, selecting 'No' will disable it.)") in list("Yes", "No")
	if (i == "Yes")
