	...
	if (isplanevalidtool(W))
		if (istype(W, /obj/item/weapon/wrench) && !user.repairing_plane)
			...
			if (do_after(user, 50, src))
				...
				master.damage = max(master.damage - heal_damage["wrench"], 0)
				user.repairing_plane = 0
			else
				user.repairing_plane = 0
			...
			if (do_after(user, 60, src))
				...
				master.damage = max(master.damage - heal_damage["weldingtool"], 0)
				user.repairing_plane = 0
			else
				user.repairing_plane = 0
	else
