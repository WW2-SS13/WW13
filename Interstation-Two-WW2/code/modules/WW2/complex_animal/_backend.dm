...
/mob/living/simple_animal/complex_animal/Move()
	...
	for (var/mob/living/L in living_mob_list)
		...
		if (istype(L, /mob/living/simple_animal/complex_animal))
			...
			if (sprob(90) && dist_x <= C.sightrange && dist_y <= C.sightrange)
				C.knows_about_mobs |= src
			else if (sprob(45) && dist_x <= C.hearrange && dist_y <= C.hearrange)
				C.knows_about_mobs |= src
			else if (sprob(22) && dist_x <= C.smellrange && dist_y <= C.smellrange)
				C.knows_about_mobs |= src
