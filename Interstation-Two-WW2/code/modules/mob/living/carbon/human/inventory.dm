...
/mob/living/carbon/human/attack_ui(slot)
	...
	var/o_rhand = r_hand
	..(slot)
	if (l_hand && l_hand != o_lhand && istype(l_hand, /obj/item/weapon/gun))
		visible_message("<span class = 'danger'>[src] draws out a gun!</span>")
		return
	if (r_hand && r_hand != o_rhand && istype(r_hand, /obj/item/weapon/gun))
