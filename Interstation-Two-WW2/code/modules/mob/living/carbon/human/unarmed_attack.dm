...
/datum/unarmed_attack/proc/apply_effects(var/mob/living/carbon/human/user,var/mob/living/carbon/human/target,var/armour,var/attack_damage,var/zone)
	...
	if(attack_damage >= 5 && armour < 2 && !(target == user) && stun_chance <= attack_damage * 5) // 25% standard chance
		switch(zone) // strong punches can have effects depending on where they hit
			...
			if("chest")
				if(!target.lying)
					...
					if(!T.density)
						...
						target.visible_message("<span class='danger'>[spick("[target] was sent flying backward!", "[target] staggers back from the impact!")]</span>")
					else
						target.visible_message("<span class='danger'>[target] slams into [T]!</span>")
						target.Weaken(srand(2,3))
						target.adjustBruteLoss(srand(10,15))
			...
			if("groin")
				target.visible_message("<span class='warning'>[target] looks like \he is in pain!</span>", "<span class='warning'>[(target.gender=="female") ? "Oh god that hurt!" : "Oh no, not your[spick("testicles", "crown jewels", "clockweights", "family jewels", "marbles", "bean bags", "teabags", "sweetmeats", "goolies")]!"]</span>")
		...
		if(!target.lying)
			target.visible_message("<span class='danger'>[target] [spick("slumps", "falls", "drops")] down to the ground!</span>")
		else
...
/datum/unarmed_attack/proc/show_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target, var/zone, var/attack_damage)
	...
	user.visible_message("<span class='warning'>[user] [spick(attack_verb)] [target] in the [affecting.name]!</span>")
...
/datum/unarmed_attack/proc/handle_eye_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target)
	...
	eyes.take_damage(srand(3,4), TRUE)
...
/datum/unarmed_attack/punch/show_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target, var/zone, var/attack_damage)
	...
	if(target == user)
		user.visible_message("<span class='danger'>[user] [spick(attack_verb)] \himself in the [organ]!</span>")
		return FALSE
		...
		switch(zone)
			if("head", "mouth", "eyes")
				...
				switch(attack_damage)
					...
					if(3 to 4)
						user.visible_message(pick(
							40; "<span class='danger'>[user] [spick(attack_verb)] [target] in the head!</span>",
							30; "<span class='danger'>[user] struck [target] in the head[spick("", " with a closed fist")]!</span>",
							30; "<span class='danger'>[user] threw a hook against [target]'s head!</span>"
							))
					if(5)
						user.visible_message(pick(
							30; "<span class='danger'>[user] gave [target] a resounding [spick("slap", "punch")] to the face!</span>",
							40; "<span class='danger'>[user] smashed \his [spick(attack_noun)] into [target]'s face!</span>",
				...
				switch(attack_damage)
					...
					if(1 to 4)	user.visible_message("<span class='danger'>[user] [spick(attack_verb)] [target] in \his [organ]!</span>")
					if(5)
						user.visible_message(pick(
							50; "<span class='danger'>[user] smashed \his [spick(attack_noun)] into [target]'s [organ]!</span>",
							50; "<span class='danger'>[user] landed a striking [spick(attack_noun)] on [target]'s [organ]!</span>"
							))
	else
		user.visible_message("<span class='danger'>[user] [spick("punched", "threw a punch against", "struck", "slammed their [spick(attack_noun)] into")] [target]'s [organ]!</span>") //why do we have a separate set of verbs for lying targets?
...
/datum/unarmed_attack/kick/show_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target, var/zone, var/attack_damage)
	...
	switch(attack_damage)
		if(1 to 2)	user.visible_message("<span class='danger'>[user] threw [target] a glancing [spick(attack_noun)] to the [organ]!</span>") //it's not that they're kicking lightly, it's that the kick didn't quite connect
		if(3 to 4)	user.visible_message("<span class='danger'>[user] [spick(attack_verb)] [target] in \his [organ]!</span>")
		if(5)		user.visible_message("<span class='danger'>[user] landed a strong [spick(attack_noun)] against [target]'s [organ]!</span>")
...
/datum/unarmed_attack/stomp/show_attack(var/mob/living/carbon/human/user, var/mob/living/carbon/human/target, var/zone, var/attack_damage)
	...
	switch(attack_damage)
		if(1 to 4)	user.visible_message("<span class='danger'>[spick("[user] stomped on", "[user] slammed \his [shoes ? copytext(shoes.name, TRUE, -1) : "foot"] down onto")] [target]'s [organ]!</span>")
		if(5)		user.visible_message("<span class='danger'>[spick("[user] landed a powerful stomp on", "[user] stomped down hard on", "[user] slammed \his [shoes ? copytext(shoes.name, TRUE, -1) : "foot"] down hard onto")] [target]'s [organ]!</span>") //Devastated lol. No. We want to say that the stomp was powerful or forceful, not that it /wrought devastation/
