...
/proc/get_rand_frequency()
	return srand(32000, 55000) //Frequency stuff only works with 45kbps oggs.

/proc/get_sfx(soundin)
	if(istext(soundin))
		switch(soundin)
			if ("shatter") soundin = spick(shatter_sound)
			if ("explosion") soundin = spick(explosion_sound)
			if ("sparks") soundin = spick(spark_sound)
			if ("rustle") soundin = spick(rustle_sound)
			if ("punch") soundin = spick(punch_sound)
			if ("clownstep") soundin = spick(clown_sound)
			if ("swing_hit") soundin = spick(swing_hit_sound)
			if ("hiss") soundin = spick(hiss_sound)
			if ("pageturn") soundin = spick(page_sound)
			if ("keyboard") soundin = spick(keyboard_sound)
			if ("robot_talk_heavy") soundin = spick(robot_talk_heavy_sound)
			if ("robot_talk_light") soundin = spick(robot_talk_light_sound)
			if ("miss_sound") soundin = spick(miss_sound)
			if ("ric_sound") soundin = spick(ric_sound)
			if ("casing_sound") soundin = spick(casing_sound)
			if ("hitobject") soundin = spick(bullet_hit_object_sound)
			if ("trauma") soundin = spick(trauma_sound)
			if ("platingfootsteps") soundin = spick(platingfootsteps)
			if ("erikafootsteps") soundin = spick(erikafootsteps)
			if ("grassfootsteps") soundin = spick(grassfootsteps)
			if ("dirtfootsteps") soundin = spick(dirtfootsteps)
			if ("waterfootsteps") soundin = spick(waterfootsteps)
			if ("sandfootsteps") soundin = spick(sandfootsteps)
			if ("woodfootsteps") soundin = spick(woodfootsteps)
			if ("carpetfootsteps") soundin = spick(carpetfootsteps)
			if ("artillery_out") soundin = spick(artillery_out)
			if ("artillery_in") soundin = spick(artillery_in)
			if ("artillery_out_distance") soundin = spick(artillery_out_distance)
			if ("artillery_in_distance") soundin = spick(artillery_in_distance)
			if ("train_movement") soundin = spick(train_movement_sounds)
			if ("train_halting") soundin = spick(train_halting_sounds)
			if ("wryyy") soundin = spick(wryyys)
			if ("doorknock") soundin = spick(doorknock_sounds)

			// emote sounds from InterBay
			if ("cough_male")
				soundin = spick(cough_sounds_male)
			if ("cry_male")
			...
			if ("sigh_male")
				soundin = spick(sigh_sounds_male)
			if ("sneeze_male")
				soundin = spick(sneeze_sounds_male)

			if ("cough_female")
				soundin = spick(cough_sounds_female)
			if ("cry_female")
			...
			if ("sigh_female")
				soundin = spick(sigh_sounds_female)
			if ("sneeze_female")
				soundin = spick(sneeze_sounds_female)
